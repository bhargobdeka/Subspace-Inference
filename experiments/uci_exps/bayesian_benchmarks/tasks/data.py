...
class Dataset(object):
    def __init__(self, split=0, prop=0.9):
        ...
        # index_train = np.loadtxt(DATA_PATH + self.name + "/index_train_{}.txt".format(split)).astype(int)
        # index_test  = np.loadtxt(DATA_PATH + self.name + "/index_test_{}.txt".format(split)).astype(int)
        
        # self.X_train = X[index_train,]
        # self.Y_train = Y[index_train]
        # self.X_test  = X[index_test,]
        # self.Y_test  = Y[index_test]
...
class Concrete(Dataset):
    ...
    url = uci_base_url + 'concrete/data_concrete.txt'
...
class Energy(Dataset):
    ...
    url = uci_base_url + 'energy/data_energy.txt'
...
class Kin8mn(Dataset):
    ...
    url = uci_base_url + 'kin8nm/data_kin8nm.txt'
...
class Naval(Dataset):
    ...
    # url = uci_base_url + 'naval/data_naval.txt'
...
class Power(Dataset):
    ...
    url = uci_base_url + 'power/data_power.txt'
    def read_data(self):
        data = np.loadtxt(self.datapath)
        # data = pandas.read_csv(self.datapath, header=None).values
        return data[:, :-1], data[:, -1].reshape(-1, 1)
...
class Protein(Dataset):
    ...
    url = uci_base_url + 'protein/data_protein.txt'
    def read_data(self):
        data = np.loadtxt(self.datapath)
        # data = pandas.read_csv(self.datapath, header=None).values
        return data[:, :-1], data[:, -1].reshape(-1, 1)
...
class WineRed(Dataset):
    ...
    url = uci_base_url + 'winered/data_winered.txt'
    def read_data(self):
        data = np.loadtxt(self.datapath)
        # data = pandas.read_csv(self.datapath, header=None).values
        return data[:, :-1], data[:, -1].reshape(-1, 1)
...
class Yacht(Dataset):
    ...
    url = uci_base_url + 'yacht/data_yacht.txt'
    ...
    def read_data(self):
        ...
        data = np.loadtxt(self.datapath)
        return data[:, :-1], data[:, -1].reshape(-1, 1)
#%% Multiple Output Regression Datasets
@add_regression
class enb(Dataset):
    N, D, name = 768, 8, 'enb'
    url = uci_base_url + 'enb/data_enb.txt'
    def read_data(self):
        data = np.loadtxt(self.datapath)
        # data = pandas.read_csv(self.datapath, header=None).values
        X = data[:, 0:8]
