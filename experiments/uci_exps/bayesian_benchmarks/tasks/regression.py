...
from get_model import get_regression_model
...
def run(ARGS, data=None, model=None, is_test=False):
    ...
    start = time.time()
    results, lltests, rmsetests = model.fit(data.X_train, data.Y_train, data.X_test, data.Y_test, data.Y_std)
