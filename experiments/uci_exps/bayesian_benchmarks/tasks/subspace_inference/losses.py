...
class GaussianLikelihood:
    ...
    def __call__(self, model, input, target):
        ...
        if self.noise_var is not None:
            ...
            mean = output[:,[0,1]].view_as(target) # to be changed for multiple variance terms
            sp   = torch.nn.Softplus()
            var  = sp(output[:,[2,3]])+(10)**-6
            var  = var.view_as(target)
            ...
            return loss.mean(), output[:,[0,1]], {'mse': torch.mean((mean - target)**2.0)}
